...
export default function SettingsPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <label className="text-xs text-white/60">Relevance Filter</label>
          <span className="text-xs text-white/80 font-mono">
            {Math.round(settings.relevanceThreshold)}%
          </span>
        </div>
        <input
          type="range"
          ...
          onChange={(e) => updateSettings({ relevanceThreshold: parseFloat(e.target.value) })}
          className="w-full h-1 bg-[#2C2C2E] rounded-lg appearance-none cursor-pointer accent-[#0A84FF]"
        />
        <div className="flex justify-between text-[10px] text-white/30">
          <span>0% (all)</span>
