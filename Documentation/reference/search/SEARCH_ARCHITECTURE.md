...
Internal note:
- NetworkX and LightRAG remain deployed as internal retrieval subsystems behind `cascading` and `deep-research`.
...
2. **Cascading**: Thorough staged retrieval pipeline. Uses the internal graph subsystem to anchor the concept, expands understanding via LightRAG, falls back to targeted vector search, and uses an LLM to synthesize a concise answer.
