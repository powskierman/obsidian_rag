...
- **How to Use**:
  1.  Read the spec (e.g., `Documentation/reference/spec.md`).
...
**Purpose**: Health check for the supported public search paths plus the internal graph-backed subsystem.
- **When to Use**: If users report "Search is broken" or "Vector works but cascading is thin".
- **How to Use**:
  1.  Run `python Scripts/debug/audit_search_modes.py`
  2.  Compare outcomes against `Documentation/reference/search/SEARCH_MODES_GUIDE.md`.
- **Example in `obsidian_rag`**:
  - *Scenario*: User says "Cascading search returns weak evidence".
  - *Action*: Run audit. If `Vector: PASS` and `Cascading: FAIL`, inspect the internal graph services behind cascading.

#### 8. RAG Eval Runner
**Purpose**: Quality assurance for retrieval accuracy.
- **When to Use**: To verify if an embedding or internal graph-subsystem change actually improved results.
- **How to Use**:
  1.  Select queries from `Documentation/operations/quality/KNOWLEDGE_GRAPH_TEST_PROMPTS.md`.
  2.  Run queries in `vector` vs `cascading` mode.
