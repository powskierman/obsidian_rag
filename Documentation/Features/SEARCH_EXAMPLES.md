...
curl -s -X POST http://localhost:4000/api/v1/search \
  ...
  -d '{"query":"ESP32 UART wiring","mode":"vector","n_results":5}'
```

```bash
curl -s -X POST http://localhost:4000/api/v1/search \
  -H "Content-Type: application/json" \
  -d '{"query":"CAR-T timeline","mode":"vector","n_results":5}'
```
...
curl -s -X POST http://localhost:4000/api/v1/search \
  -H "Content-Type: application/json" \
  -d '{"query":"nextion esp32 update process","mode":"hybrid","n_results":5}'
```
