...
_OBSIDIAN_TEMPLATE_EXPR_PATTERN = re.compile(r"<%[\s\S]*?%>")
_MARKDOWN_HEADING_PATTERN = re.compile(r"^\s{0,3}#{1,6}\s+.+$")
_EMPTY_BULLET_PATTERN = re.compile(r"^\s*[-*+]\s*$")
_EMPTY_CHECKBOX_PATTERN = re.compile(r"^\s*[-*+]\s+\[[ xX]\]\s*$")


def _is_placeholder_markdown_line(line: str) -> bool:
    """Return True for markdown lines that carry no meaningful note content."""
    stripped = line.strip()
    if not stripped:
        return True
    if stripped in {"`", "``", "```"}:
        return True
    if _EMPTY_BULLET_PATTERN.match(stripped):
        return True
    if _EMPTY_CHECKBOX_PATTERN.match(stripped):
        return True
    return False


def preprocess_markdown_for_indexing(content: str) -> str:
    """Remove common Obsidian template artifacts before indexing."""
    if not content:
        return content

    normalized = content.replace("\r\n", "\n").replace("\r", "\n")
    normalized = _OBSIDIAN_TEMPLATE_EXPR_PATTERN.sub("", normalized)
    ...
    while i < len(lines):
        ...
        while i < len(lines) and not _MARKDOWN_HEADING_PATTERN.match(lines[i]):
            section_body.append(lines[i].rstrip())
            i += 1

        kept_body = [
            body_line
            for body_line in section_body
            if body_line.strip() and not _is_placeholder_markdown_line(body_line)
        ]
        if kept_body:
            cleaned_lines.append(heading_line)
            cleaned_lines.extend(kept_body)
