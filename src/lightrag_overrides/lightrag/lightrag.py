...
class LightRAG:
    ...
    enable_markdown_preprocessing: bool = field(
        default=get_env_value("ENABLE_MARKDOWN_PREPROCESSING", True, bool)
    )
    """If True, strip markdown template artifacts and empty placeholder-only sections before indexing."""
    ...
    async def apipeline_enqueue_documents(
        ...
        if ids is not None:
            ...
            for id_, doc, path in zip(ids, input, file_paths):
                ...
                if self.enable_markdown_preprocessing:
                    cleaned_content = preprocess_markdown_for_indexing(cleaned_content)
