    ...
    **kwargs,
    ...
    if timeout is None:
        ...
        if env_timeout:
            try:
                ...
                logger.warning(
                    f"Invalid OLLAMA_TIMEOUT value '{env_timeout}', using client default"
                )
    if timeout == 0:
        timeout = None
    ...
    try:
        ...
        if not isinstance(request, httpx.Request):
            request = httpx.Request("POST", host or "http://localhost")

        if isinstance(e, (httpx.ReadTimeout, httpx.TimeoutException, TimeoutError)):
            raise APITimeoutError(request) from e
        if isinstance(e, (httpx.ConnectError, httpx.ConnectTimeout)):
            raise APIConnectionError(message=str(e), request=request) from e
