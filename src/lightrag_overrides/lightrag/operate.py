...
async def extract_entities(
    ...
    chunk_retry_attempts = int(
        global_config.get(
            "entity_extract_chunk_retry_attempts",
            os.getenv("ENTITY_EXTRACT_CHUNK_RETRY_ATTEMPTS", 2),
        )
    )
    chunk_retry_backoff_seconds = float(
        global_config.get(
            "entity_extract_chunk_retry_backoff_seconds",
            os.getenv("ENTITY_EXTRACT_CHUNK_RETRY_BACKOFF_SECONDS", 2.0),
        )
    )

    skip_failed_chunks_raw = str(
        global_config.get(
            "entity_extract_skip_failed_chunks",
            os.getenv("ENTITY_EXTRACT_SKIP_FAILED_CHUNKS", "true"),
        )
    ).strip()
    skip_failed_chunks = skip_failed_chunks_raw.lower() in {"1", "true", "yes", "on"}
    ...
    def _is_retryable_timeout(exc: Exception) -> bool:
        ...
        for current in _iter_exception_chain(exc):
            if type(current).__name__ in timeout_type_names:
                return True
            if isinstance(current, (asyncio.TimeoutError, WorkerTimeoutError, APITimeoutError)):
                return True
    ...
    async def _process_with_semaphore(chunk):
        ...
        async with semaphore:
            ...
            try:
                max_attempts = max(0, chunk_retry_attempts) + 1
                for attempt in range(1, max_attempts + 1):
                    try:
                        ...
                        chunk_id = chunk[0]
                        is_retryable_timeout = _is_retryable_timeout(e)
                        is_last_attempt = attempt >= max_attempts

                        if is_retryable_timeout and not is_last_attempt:
                            backoff = chunk_retry_backoff_seconds * (2 ** (attempt - 1))
                            logger.warning(
                                f"{chunk_id}: timeout/retryable error on attempt {attempt}/{max_attempts}; retrying in {backoff:.1f}s. Error: {e}"
                            )
                            await asyncio.sleep(backoff)
                            continue

                        prefixed_exception = create_prefixed_exception(e, chunk_id)
                        if skip_failed_chunks and is_retryable_timeout:
                            failed_chunks += 1
                            failed_chunk_ids.append(chunk_id)
                            logger.error(
                                f"Skipping timed-out chunk {chunk_id} after {attempt}/{max_attempts} attempt(s): {e}"
                            )
                            if pipeline_status is not None and pipeline_status_lock is not None:
                                async with pipeline_status_lock:
                                    msg = f"Skipped timed-out chunk {chunk_id} after retries"
    ...
    if not chunk_results:
        if failed_chunk_ids:
            raise RuntimeError(
                f"All chunks failed during extraction. Failed chunks: {', '.join(failed_chunk_ids)}"
            )
        raise RuntimeError("Entity extraction produced no chunk results.")

    if failed_chunks > 0:
        logger.warning(
            f"Entity extraction completed with partial success: {len(chunk_results)}/{total_chunks} chunks succeeded, {failed_chunks} failed/skipped."
        )
