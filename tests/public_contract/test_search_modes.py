...
@pytest.mark.integration
def test_vector_mode_passes_web_search_into_synthesis(monkeypatch):
    monkeypatch.setenv("TAVILY_API_KEY", "test-key")
    routes = {
        f"{api_gateway.EMBEDDING_SERVICE_URL}/query": lambda _json: FakeResponse(
            {
                "documents": [["P1S has a 256 x 256 x 256 mm build volume."]],
                "metadatas": [[{"filename": "bambu-lab-P1S-tech-specs.pdf", "filepath": "Tech/3D-Printing/media/bambu-lab-P1S-tech-specs.pdf"}]],
                "distances": [[0.2]],
            }
        ),
        "https://api.tavily.com/search": lambda payload: FakeResponse(
            {
                "results": [
                    {
                        "title": "Bambu Lab P1S vs Creality CR-10 SE",
                        "url": "https://example.com/p1s-vs-cr10",
                        "content": "The CR-10 line offers a larger build volume, while the P1S emphasizes speed and enclosure.",
                    }
                ],
                "query": payload["query"],
            }
    ...
    async def _fake_compressor(**kwargs):
        captured["supplemental_context"] = kwargs.get("supplemental_context")
        return {
            "answer": "The P1S has a smaller enclosed build area, while supplemental web evidence says the CR-10 line is larger.",
            "citations": ["[[Tech/3D-Printing/media/bambu-lab-P1S-tech-specs.pdf]]"],
        }
    ...
    response = _post_query(
        client,
        "vector",
        query="What is the difference between the bambu p1s and the creality cr-10",
        web_search=True,
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["web_search"]["results"][0]["title"] == "Bambu Lab P1S vs Creality CR-10 SE"
    assert "Supplemental web evidence:" in captured["supplemental_context"]
    assert "larger build volume" in captured["supplemental_context"]
    assert any(call["url"] == "https://api.tavily.com/search" for call in calls)


@pytest.mark.integration
def test_cascading_mode_fetches_web_search_before_synthesis(monkeypatch):
    monkeypatch.setenv("TAVILY_API_KEY", "test-key")
    routes = {
        "https://api.tavily.com/search": lambda payload: FakeResponse(
            {
                "results": [
                    {
                        "title": "Derivative asymptote relation",
                        "url": "https://example.com/asymptote-derivative",
                        "content": "A tangent at infinity can define an oblique asymptote.",
                    }
                ],
                "query": payload["query"],
            }
    ...
    async def _fake_synth(query, sources, llm_provider, model, system_prompt, brief_concept_index, supplemental_context=None):
        captured["supplemental_context"] = supplemental_context
        return {
            "answer": "Supplemental web evidence helps answer the relation.",
            "citations": ["[[Math/Asymptotes.md]]"],
    ...
    response = _post_query(
        client,
        "cascading",
        query="How do asymptotes relate to derivatives",
        web_search=True,
    )

    assert response.status_code == 200
    data = response.json()
    assert data["web_search"]["results"][0]["title"] == "Derivative asymptote relation"
    assert "Supplemental web evidence:" in captured["supplemental_context"]
    assert any(call["url"] == "https://api.tavily.com/search" for call in calls)


@pytest.mark.integration
def test_cascading_mode_hydrates_returned_sources_from_synthesis(monkeypatch):
    class FakeRetriever:
        def __init__(self, *args, **kwargs):
            pass
    ...
    async def _fake_synth(query, sources, llm_provider, model, system_prompt, brief_concept_index, supplemental_context=None):
        return {
            "answer": "Asymptotes relate to derivatives through limiting slope behavior.",
            "citations": ["[[Math/Asymptotes.md]]"],
            "used_documents": [
                {
                    "filename": "Asymptotes.md",
                    ...
                    "content": "Asymptotes describe a line that a curve approaches while derivatives describe slope.",
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["sources"][0]["snippet"].startswith("Asymptotes describe a line")
    assert data["results"]["used_documents"][0]["snippet"].startswith("Asymptotes describe a line")


@pytest.mark.integration
def test_cascading_mode_passes_requested_model_to_lightrag(monkeypatch):
    captured = {}
    ...
    async def _fake_synth(query, sources, llm_provider, model, system_prompt, brief_concept_index, supplemental_context=None):
        return {
            "answer": "Asymptotes relate to derivatives through limiting behavior.",
            "citations": ["[[Math/Asymptotes.md]]"],
            "used_documents": sources,
        }
    ...
    assert response.status_code == 200
    assert captured["llm_provider"] == "openrouter"
    assert captured["llm_model"] == "z-ai/glm-5"
